...
# Function for getting user GUID. Input is user email address
Function GetUserGuidFromEmail
(
    [String] $UserEmail
)
{
   ...
   <soapenv:Body>
      <adm:GetUsersRequest>
         ...
         <adm:searchCriteria>
            <adm:emailAddress>'+$UserEmail+'</adm:emailAddress>         </adm:searchCriteria>
         <adm:sortBy>
            <adm:DISPLAY_NAME>true</adm:DISPLAY_NAME>
            <adm:USER_UID>false</adm:USER_UID>
            <adm:USER_NAME>false</adm:USER_NAME>
...
# Function to get some basic user info from UEM based on samaccountname
...
)
{
   ...
   <soapenv:Body>
    ...
    $soap_object = [xml]$soapvar
    $Response=(Soap_Request_Basicauth -uname (GetEncodedUserName($UEMUser)) -password $UEMPassword -Soap_Request ($soap_object.Innerxml) -URL $BWSUrl) 
    $Status=($response.Envelope.Body.GetUsersResponse.users) 
    return $Status
}

# Function to get some basic user info from UEM based on Email address
Function GetUserInfoFromEmail
(
    [String] $UserEmail
)
{
