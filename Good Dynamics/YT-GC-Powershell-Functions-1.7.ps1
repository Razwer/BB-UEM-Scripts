...
# Change PolicySet for User. Input email and policyset id
function ChangePolicyUser
( 
        [String] $UserName,
        [string] $PolicySetID
) 
{
    ...
    $UserID = (Get-Userid $UserName)
    # Build envelope with ID's
    $ChangePolicyUserEnvelope = (Build-Envelope "ChangePolicyUserRequest" "policySetId" $PolicySetID "userId" $UserId)
    $ChangePolicyUserRequest = (Execute-SOAPRequest $ChangePolicyUserEnvelope $url)
    #$ChangePolicyUser = ($ChangePolicyUserRequest.Envelope.Body.ChangePolicyUserResponse)
    #return $ChangePolicyUser  
} 
...
# Change policy set for user. Input email and policyset id. To find out the policy set id's run Get-AllPolicies
#ChangePolicyUser "jon.jaffe@gdbeslab.com" "48352"