...
# Add MASTER Connection Profile Rule
...
) 
{
    ...
    # Start the envelope
    $AddProfileRuleEnvelope = (Build-EnvelopeStartNoSub "UpdateConnectionProfileRulesRequest" "connectionProfileId" "1")
...
#Add-ProfileRule "blabla.com:443" "First" "Second"

# Add connections to an app. Second cluster is optional
...
#############################
# EDIT BELOW FOR DEPLOYMENT #
#############################

$GemsServers = ("gems001.contoso.local","gems002.contoso.local") # array of all Gems servers
$CasServer = "casarray01.contoso.local" # Hostname of CAS array
$PrimaryGPClusterName = "First" #Primary GP cluster name.
#$SecondaryGPClusterName = "Second" #Secondary GP cluster name. Comment out if not required.
$GWAppID = "com.good.gcs.g3" #Good Work. Comment out if not required.
$DocsAppId = "com.good.feature.share" #Docs Feature. Comment out if not required
$GoodConnectAppId = "com.good.goodconnect" #Good Connect. Comment out if not required
$GoodEnterpriseServicesAppId = "com.good.gdservice-entitlement.enterprise" #Enterprise Services. Comment out if not required

ForEach ($gems in $GemsServers) {
    # Add all gems to master connection profile with 8443
    Add-ProfileRule "$gems:8443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"

    # Add cas server as well
    Add-ProfileRule "$CasServer:443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"

    # If Good Connect is used also add 8080. While we are at it, add app rule as well
    if ($GoodConnectAppId) { 
        Add-ProfileRule "$gems:8080" "$PrimaryGPClusterName" "$SecondaryGPClusterName"
        Add-AppRule "$GoodConnectAppId" "$gems" "8080" "$PrimaryGPClusterName" "$SecondaryGPClusterName"  
        }
    # Add Good Work App rules. Also add CAS
    if ($GWAppID) { 
        Add-AppRule "$GWAppID" "$gems" "8443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"
        Add-AppRule "$CasServer" "$gems" "443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"    
    }

    # Add Docs app rules
    if ($DocsAppId) { 
        Add-AppRule "$DocsAppId" "$gems" "8443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"
        }

    # Add Enterprise Services app rules
    if ($GoodEnterpriseServicesAppId) { 
        Add-AppRule "$GoodEnterpriseServicesAppId" "$gems" "8443" "$PrimaryGPClusterName" "$SecondaryGPClusterName"
        }
}